...
class from_py_model(classification_model):
    ...
    def _forward_pass(self, inputs, trainable=True, dropout_keep_prob=0.4,num_classes=1000, restore_logits=None, scope=''):
        '''
        ...
        with tf.name_scope(scope, "googlenet", [inputs]):
            with ops.arg_scope([layers.max_pool2d], padding='SAME'):
                ...
                with tf.variable_scope("inception_3a"):
                    end_points['inception_3a'] = self.get_inception_layer(
                        end_points['pool1'], 64, 96, 128, 16, 32, 32)
                with tf.variable_scope("inception_3b"):
                    end_points['inception_3b'] = self.get_inception_layer(
                ...
                with tf.variable_scope("inception_4a"):
                    end_points['inception_4a'] = self.get_inception_layer(
                        end_points['pool2'], 192, 96, 208, 16, 48, 64)
                with tf.variable_scope("aux_logits_1"):
                    end_points['aux_logits_1'] = self.aux_logit_layer(
                        end_points['inception_4a'], num_classes, trainable)
                with tf.variable_scope("inception_4b"):
                    end_points['inception_4b'] = self.get_inception_layer(
                        end_points['inception_4a'], 160, 112, 224, 24, 64, 64)
                with tf.variable_scope("inception_4c"):
                    end_points['inception_4c'] = self.get_inception_layer(
                        end_points['inception_4b'], 128, 128, 256, 24, 64, 64)
                with tf.variable_scope("inception_4d"):
                    end_points['inception_4d'] = self.get_inception_layer(
                        end_points['inception_4c'], 112, 144, 288, 32, 64, 64)
                with tf.variable_scope("aux_logits_2"):
                    end_points['aux_logits_2'] = self.aux_logit_layer(
                        end_points['inception_4d'], num_classes, trainable)
                with tf.variable_scope("inception_4e"):
                    end_points['inception_4e'] = self.get_inception_layer(
                ...
                with tf.variable_scope("inception_5a"):
                    end_points['inception_5a'] = self.get_inception_layer(
                        end_points['pool3'], 256, 160, 320, 32, 128, 128)
                with tf.variable_scope("inception_5b"):
                    end_points['inception_5b'] = self.get_inception_layer(
                ...
                end_points['dropout'] = layers.dropout(
                    end_points['reshape'], dropout_keep_prob, is_training=trainable)
